...
char* createHash(char salt[16], char pepper[], char password[16])
{
	std::string str = "";
	for (int i = 0; password[i + 1] != NULL; i++) str += password[i];
	for (int i = 0; i < 32; i = i + 2) str += salt[i];
	for (int i = 0; i < 32; i = i + 2) str += pepper[i];
	SHA256 sha;
	sha.update(str);
	std::array<uint8_t, 32> digest = sha.digest();
	std::string hashed = SHA256::toString(digest);
	char* hash = new char[64];
	for (int i = 0; i < 64; i++) hash[i] = hashed[i];
	return hash;
}
